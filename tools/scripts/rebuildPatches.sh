...
rebuildPatches() {
    ...
    for javaFile in $(find "./src/main/java/net/minecraft/" -name "*.java")
    do
        javaFileClean=${javaFile#"./src/main/java/net/minecraft/"}
        patchOutPath="./patches/"$(echo $javaFileClean | cut -d. -f1)".patch"
        ...
        strip_cr "./src/main/java/net/minecraft/$javaFileClean" > /dev/null
        
        difference=$(diff -u --label a/net/minecraft/$javaFileClean ".cache/$mcVersion/decompiled/net/minecraft/$javaFileClean" --label b/net/minecraft/$javaFileClean "./src/main/java/net/minecraft/$javaFileClean")
